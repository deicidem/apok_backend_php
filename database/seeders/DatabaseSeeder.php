...
use App\Models\User;
use App\Notifications\DzzRequestNotification;
...
class DatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        Group::create([
            'title'    => 'ЦИТ 1',
            'description' => 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Molestiae quisquam nisi expedita beatae blanditiis totam, repellat et alias consectetur odio quidem reiciendis cum quo ducimus rem consequuntur suscipit, dolores similique.',
            'owner_id' => 1,
            'type_id'     => 1
        ]);
        Group::create([
        ...
        $json3    = json_decode(Storage::get('public/result/Превью векторов.geojson'));
        $polygon3 = json_encode(\GeoJson\GeoJson::jsonUnserialize($json3)->getFeatures()[0]->getGeometry()->jsonSerialize());
        TaskResultView::Create([
        ...
        User::find(1)->notify(new GroupNotification("Вы были добавлены в группу «Центр иновационных технологий»"));
        User::find(1)->notify(new GroupNotification("Вы были исключены из группы «Центр иновационных технологий»"));
        User::find(1)->notify(new TaskNotification("Задача «Оценка координатно-измерительных характеристик» завершена"));
        User::find(1)->notify(new TaskNotification("Задача «Оценка пространственно-частотных характеристик (оптика)» отменена"));
        User::find(1)->notify(new DzzRequestNotification("Запрос на снимки успешно отправлен"));
        User::find(1)->notify(new DzzRequestNotification("Получен результат по запросу на снимки"));
        User::find(1)->notify(new GroupNotification("Вы были добавлены в группу «Центр иновационных технологий»"));
        User::find(1)->notify(new GroupNotification("Вы были исключены из группы «Центр иновационных технологий»"));
        User::find(1)->notify(new TaskNotification("Задача «Оценка координатно-измерительных характеристик» завершена"));
        User::find(1)->notify(new TaskNotification("Задача «Оценка пространственно-частотных характеристик (оптика)» отменена"));
